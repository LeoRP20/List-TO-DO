...
function addTask() {
  let task
  let task_id
  if (document.getElementById('task').value == '') return null

  localStorage.last_index =
    localStorage.last_index == undefined
      ? 1
...
function getDataFromLocalStorage() {
  if (localStorage.last_index == undefined || localStorage.last_index == 0)
    return null
